...
import {
  Home, FilePlus2, Briefcase, ListChecks,
...
export default function MentorNavbar() {
  ...
  const navLinks = [
    { name: 'Dashboard', path: '/mentor/dashboard', icon: <Home size={15} /> },
    { name: 'Add Profile', path: '/mentor/create-profile', icon: <FilePlus2 size={15} /> },
    { name: 'My Profiles', path: '/mentor/profiles', icon: <Briefcase size={15} /> },
    { name: 'Submissions', path: '/mentor/submissions', icon: <ListChecks size={15} /> },
  ];
  ...
  const LOGO_BORDER = isDark ? '#ffffff' : '#080c14';
  ...
  return (
    <>
      <style>{`
        ...
        .mn-nav * { font-family: 'Plus Jakarta Sans', system-ui, sans-serif; }
        .mn-nav-blur { backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); }
        html,body,#root { overflow-x: hidden; scrollbar-width: none; -ms-overflow-style: none; }
        *::-webkit-scrollbar { display: none; width: 0; height: 0; }
        * { scrollbar-width: none; -ms-overflow-style: none; }
      `}</style>

      <nav className="mn-nav fixed top-0 w-full z-[100] transition-all duration-300 mn-nav-blur"
              ...
              return (
                <NavLink key={link.name} to={link.path}>
                  <div className="relative px-4 py-2 rounded-xl flex items-center gap-1.5 transition-colors duration-150"
                    ...
                    {active && (
                      <motion.div layoutId="mentorActiveTab"
              ...
              <AnimatePresence>
                {dropdownOpen && (
                  <motion.div
                    ...
                    style={{ background: DROP_BG, border: `1px solid ${DROP_BDR}`, boxShadow: '0 16px 48px rgba(0,0,0,0.5), 0 4px 12px rgba(14,165,233,0.08)' }}>
            ...
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}
              ...
              <div className="p-4 space-y-1">
                {navLinks.map(link => {
                  const active = location.pathname === link.path;
                  return (
                    <NavLink key={link.name} to={link.path} onClick={() => setMobileOpen(false)}>
                      <div className="flex items-center gap-3 px-4 py-3 rounded-xl transition-all"
                        style={{
                          background: active ? ACTIVE_BG : 'transparent',
                          borderLeft: active ? `3px solid ${ACTIVE_LINE}` : '3px solid transparent',
                          color: active ? LINK_ACTIVE : '#64748b',
                        }}>
                        {link.icon}
        ...
        {showLogoutModal && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              <h3 className="text-xl font-black mb-2 text-slate-100" style={{ letterSpacing: '-0.02em' }}>
                Sign out?
              </h3>
              <p className="text-sm font-medium mb-7 text-slate-500">
                You'll need to log back in to access your mentor account.
              </p>
